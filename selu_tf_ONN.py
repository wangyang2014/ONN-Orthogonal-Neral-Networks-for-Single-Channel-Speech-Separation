...
weights = {
    'wd1': tf.Variable(tf.random_normal([n_input, n_input])/n_input),
    'wd2': tf.Variable(tf.random_normal([n_input, n_input])/n_input),
    'wd_1_ST': tf.Variable(tf.random_normal([n_input])),
    #'wd_2_ST': tf.Variable(tf.random_normal([2*n_input]))*0.01,
    'codelayer_1': tf.Variable(tf.random_normal([n_input, n_input])/n_input),
    'codelayer_2': tf.Variable(tf.random_normal([n_input, n_input])/n_input),
    'wd3': tf.Variable(tf.random_normal([n_input, n_input]))/n_input,
    'out': tf.Variable(tf.random_normal([n_input, n_input])/n_input),
}
...
biases = {
    ...
    'codelayer_b1': tf.Variable(tf.random_normal([n_input])),
    'codelayer_b2': tf.Variable(tf.random_normal([n_input])),
...
def ONNModel(indata):
    #encode layer
    layer_1_out = fullConnect(indata,weights['wd1'],biases['bc1'],tf.nn.selu)
    layer_2_out = fullConnect(layer_1_out,weights['wd2'],biases['bc2'],tf.nn.selu)
    ...
    #Orthogonal
    speaker_1_Code = fullConnect(oNNSpeaker_1_Transformation,weights['codelayer_1'],biases['codelayer_b1'],tf.nn.selu)
    speaker_2_Code = fullConnect(oNNSpeaker_2_Transformation,weights['codelayer_2'],biases['codelayer_b2'],tf.nn.selu)
    ...
    layer_4_out_1 = fullConnect(layer_3_out_1,weights['out'],0,tf.nn.selu)

    layer_3_out_2 = fullConnect(out_code_2,weights['wd3'],0,tf.nn.tanh)
    layer_4_out_2 = fullConnect(layer_3_out_2,weights['out'],0,tf.nn.selu)

    #return layer_4_out_1,out_code_1,out_code_2
    return layer_4_out_1,layer_4_out_2,layer_3_out_1,layer_3_out_2
...
#cross_entropy = tf.reduce_mean(10 ** -12/ (out + 10 ** -12)) #

cost = tf.reduce_sum(tf.abs(otdata - out)) + 10 * (orthogonal_err + rata ) #+ loss_add + set_loss
...
def setTrainData(train_filelist):
    ...
    #signal_Mix_Train = np.sqrt(np.abs(signal_Mix_Train)) * np.sign(signal_Mix_Train)
    signal_S1_Train = np.sqrt(np.abs(signal_S1_Train)) * np.sign(signal_S1_Train)
    signal_S2_Train = np.sqrt(np.abs(signal_S2_Train)) * np.sign(signal_S2_Train)
...
def setTestData(standard_scaler,test_Filelist):
    ...
    #signal_Mix_Test = np.sqrt(np.abs(signal_Mix_Test)) * np.sign(signal_Mix_Test)
